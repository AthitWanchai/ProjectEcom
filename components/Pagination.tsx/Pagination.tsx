...
import SVGLeft from "../../src/app/assets/SVGLeft";
import SVGLeft2 from "../../src/app/assets/SVGLeft2";
import SVGRight2 from "../../src/app/assets/SVGRight2";
import SVGRight from "../../src/app/assets/SVGRight";
...
const Pagination = ({ pagin, onPageChange, className = "", size = "full", dataLength = 0, containerClassName = "" }: IProps) => {
  ...
  const [page, setPage] = useState<number>(pagin.currentPage);
  ...
  const renderPageNumbers = (className = "") => {
    ...
    return pageNumbers.map((page: number) => (
      ...
      >
        <label className={`${pagin.currentPage === page && "text-[#FD6655] underline underline-offset-2"} text-md cursor-pointer font-semibold`}>{formatNumber(page, 0)}</label>
      </button>
    ));
  };
    ...
    <div className={dataLength === 0 ? "hidden" : `${containerClassName} flex flex-wrap md:justify-between justify-center w-full my-5 gap-5`}>
      ...
      <div className="flex flex-nowrap md:flex-none items-center justify-end gap-x-1 gap-y-3 ">
        <div className={`${pagin.totalPages > 0 ? "pagination" : "hidden"} flex gap-1 items-center`}>
          ...
          >
            <SVGLeft2 className={`${pagin.currentPage === 1 ? "stroke-[#8D8D8D] cursor-not-allowed" : "stroke-blue-main cursor-pointer"}`} width={16} height={16} />
          </div>
          <div
            className={`${pagin.currentPage === 1 ? "cursor-not-allowed" : "cursor-pointer"}`}
            onClick={() => {
          ...
          >
            <SVGLeft className={`${pagin.currentPage === 1 ? "stroke-[#8D8D8D] cursor-not-allowed" : "stroke-blue-s1 cursor-pointer"}`} width={16} height={16} />
          </div>
          ...
          >
            <SVGRight className={`${pagin.currentPage === pagin.totalPages ? "stroke-[#8D8D8D] cursor-not-allowed" : "stroke-blue-main cursor-pointer"}`} width={16} height={16} />
          ...
          >
            <SVGRight2 className={`${pagin.currentPage === pagin.totalPages ? "stroke-[#8D8D8D] cursor-not-allowed" : "stroke-blue-main cursor-pointer"}`} width={16} height={16} />
          </div>
        </div>
        <div className={`${pagin.totalPages > 0 && size === "full" ? "flex w-full sm:w-fit justify-end items-center px-2" : "hidden"}`}>
          <label className="pr-2 whitespace-nowrap text-md">ไปยังหน้าที่</label>
          <input
            type="number"
            min={1}
            value={page}
